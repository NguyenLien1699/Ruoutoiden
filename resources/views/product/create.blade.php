...
@section('title_page', isset($id) ? 'Cập nhật sản phẩm' : 'Thêm sản phẩm')
...
<div class="col-12">
    {{ Form::open(array('class' =>"card",'method' => "post", 'enctype' => "multipart/form-data")) }}
        <div class="card-header">
            <h3 class="card-title">{{ isset($id) ? 'Cập nhật sản phẩm' : 'Thêm sản phẩm' }}</h3>
        </div>
        <div class="card-body">
            <div class="row">
                <div class="col-lg-4">
                    <div class="form-group">
                        {{ Form::label('thumb', 'Avatar Nhỏ', array('class' => 'form-label')) }}
                    ...
                    <div class="form-group">
                        {{ Form::label('thumb_large', 'Avatar lớn', array('class' => 'form-label')) }}
                        {{ Form::file('thumb_large', $attributes = array('style' => "display: none;", 'target'=>'imgThumbLarge')) }}
                        @if($errors->has('thumb_large'))
                            <div class="invalid-feedback">{{$errors->first('thumb_large')}}</div>
                        @endif
                    </div>
                    <label class="imagecheck mb-4" for="thumb_large">
                        <figure class="imagecheck-figure">
                            <img src="{{ isset($thumbnail_large) ? $thumbnail_large : '/images/uploadimage.jpg' }}" alt="" class="imagecheck-image" id="imgThumbLarge">
                        </figure>
                    </label>
                </div>
                <div class="col-lg-8">
                    <div class="form-group">
                        {{ Form::label('title', 'Tên sản phẩm', array('class' => 'form-label')) }}
                    ...
                    <div class="form-group">
                        {{ Form::label('price', 'Giá bán', array('class' => 'form-label')) }}
                        {{ Form::text('price', request()->price ?? isset($price) ? $price : null, $attributes = array('class' => 'form-control'.($errors->has('price')?' is-invalid':''))) }}
                        @if($errors->has('price'))
